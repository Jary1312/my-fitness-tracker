...
function useSwipeBack(onBack, enabled = true) {
  ...
  const onTouchEnd = (e) => {
    ...
    // Swipe right ≥ 60px, started within 30px of left edge, more horizontal than vertical
    if (dx >= 60 && startX.current <= 30 && dy < 80) {
      onBack();
    }
    startX.current = null;
  };
...
function ExerciseDetail({ ex, onChange, onDelete, onBack, units, t }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",minHeight:"100vh",background:"#0a0a14"}} {...swipe}>
      <div style={{flexShrink:0,height:"max(env(safe-area-inset-top,44px),44px)"}}/>
      <div style={{flexShrink:0,padding:"4px 16px 0"}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#FF6B35",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:4,padding:"8px 0",fontFamily:"inherit"}}>← {t.back}</button>
      </div>
...
function MeasurementDetail({ meas, onChange, onDelete, onBack, units, t }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",minHeight:"100vh",background:"#0a0a14"}} {...swipe}>
      <div style={{flexShrink:0,height:"max(env(safe-area-inset-top,44px),44px)"}}/>
      <div style={{flexShrink:0,padding:"4px 16px 0"}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#00D4AA",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:4,padding:"8px 0",fontFamily:"inherit"}}>← {t.back}</button>
      </div>
      <div style={{flex:1,padding:"0 16px"}}>
      <ProgressStat entries={entries} period={period} totalProgress={totalProgress} t={t} accentColor="#00D4AA"/>
      <div style={{background:"#111120",border:"1px solid #1e1e30",borderRadius:20,padding:"18px 10px 10px",marginBottom:14}}>
...
function ExercisesView({ folder, onUpdate, onBack, units, t }) {
  ...
  if (selectedEx && ex) return (
    <div>
      <div style={{flexShrink:0}}>
        <div style={{height:"max(env(safe-area-inset-top,44px),44px)"}}/>
        <div style={{padding:"0 24px 4px"}}>
          <div style={{fontSize:10,letterSpacing:3,color:"#FF6B35",textTransform:"uppercase",marginBottom:2}}>{t.exercises}</div>
          <h1 style={{margin:0,fontSize:24,fontWeight:700,letterSpacing:-0.5,background:"linear-gradient(135deg,#fff 40%,#888)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{ex.name}</h1>
        </div>
      </div>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",minHeight:"100vh",background:"#0a0a14"}}>
      <div style={{flexShrink:0,height:"max(env(safe-area-inset-top,44px),44px)"}}/>
      <div style={{flexShrink:0,padding:"4px 24px 0"}} {...swipeBack}>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#FF6B35",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:4,padding:"8px 0",fontFamily:"inherit"}}>← {t.back}</button>
      </div>
      <div style={{flexShrink:0,padding:"0 24px 16px"}}>
        ...
        <h1 style={{margin:0,fontSize:26,fontWeight:700,letterSpacing:-0.5,background:"linear-gradient(135deg,#fff 40%,#888)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{t.exercises}</h1>
      </div>
  ...
  if (selectedM && m) return (
    <div>
      <div style={{flexShrink:0}}>
        <div style={{height:"max(env(safe-area-inset-top,44px),44px)"}}/>
        <div style={{padding:"0 24px 4px"}}>
          <div style={{fontSize:10,letterSpacing:3,color:"#00D4AA",textTransform:"uppercase",marginBottom:2}}>{t.measurements}</div>
          <h1 style={{margin:0,fontSize:24,fontWeight:700,letterSpacing:-0.5,background:"linear-gradient(135deg,#fff 40%,#888)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{m.name}</h1>
        </div>
      </div>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",minHeight:"100vh",background:"#0a0a14"}}>
      ...
      <div style={{flexShrink:0,padding:"0 24px 16px"}}>
        ...
        <h1 style={{margin:0,fontSize:26,fontWeight:700,letterSpacing:-0.5,background:"linear-gradient(135deg,#fff 40%,#888)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{t.measurements}</h1>
      </div>
      <div style={{padding:"0 16px"}}>
        {measurements.map(mx => {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",minHeight:"100vh",background:"#0a0a14"}}>
      ...
      <div style={{flexShrink:0,padding:"0 24px 16px"}}>
        ...
        <h1 style={{margin:0,fontSize:28,fontWeight:700,letterSpacing:-0.8,background:"linear-gradient(135deg,#fff 40%,#888)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{folder.name}</h1>
      </div>
...
function SettingsScreen({ lang, units, onSwitchLang, onSetUnits, onClearData, onBack, t, nickname, onSetNickname }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#0a0a14",overflow:"hidden"}} {...swipeBack}>
      <div style={{flexShrink:0,height:"max(env(safe-area-inset-top,44px),44px)"}}/>
      <div style={{flexShrink:0,padding:"4px 16px 0",background:"#0a0a14"}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#FF6B35",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:4,padding:"8px 0",fontFamily:"inherit"}}>← {t.back}</button>
      </div>
      <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:"0 16px",paddingBottom:40}}>
...
// ─── MAIN APP ─────────────────────────────────────────────────────────────────
// scrollable style for inner screens (exercises, settings etc.)
const appStyle = {
  minHeight:"100vh", background:"#0a0a14", color:"#e8e8f0",
  fontFamily:"'SF Pro Display',-apple-system,BlinkMacSystemFont,sans-serif",
  maxWidth:430, margin:"0 auto", paddingBottom:48,
};
// fixed style for home screen — position:fixed blokuje scroll w PWA standalone na iOS
const homeStyle = {
...
export default function App() {
  ...
  if (showSettings) return (
    <div style={appStyle}>
      {/* Safe-area spacer — pushes content below Dynamic Island / notch */}
      <div style={{height:"env(safe-area-inset-top, 54px)", minHeight:54}} />
  ...
  return (
    <div style={homeStyle}>
      {/* Safe-area spacer — sits below Dynamic Island */}
      <div style={{flexShrink:0, height:"max(env(safe-area-inset-top, 54px), 54px)"}} />

      {/* Header — fixed, never scrolls */}
      <div style={{flexShrink:0, padding:"10px 24px 0",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
      ...
      {/* Folders — scrollable area fills remaining space */}
