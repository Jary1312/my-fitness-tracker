...
function ExerciseDetail({ ex, onChange, onDelete, onBack, units, t }) {
  ...
  return (
    <div {...swipe}>
      <div style={{height:"max(env(safe-area-inset-top,54px),54px)"}}/>
      <div style={{position:"sticky",top:0,zIndex:10,background:"#0a0a14",padding:"4px 16px 0"}}>
...
function ExercisesView({ folder, onUpdate, onBack, units, t }) {
  ...
  return (
    <div>
      <div>
        <div style={{height:"max(env(safe-area-inset-top,54px),54px)"}}/>
        <div style={{position:"sticky",top:0,zIndex:10,background:"#0a0a14",padding:"4px 24px 0"}} {...swipeBack}>
          <button onClick={onBack} style={{background:"none",border:"none",color:"#FF6B35",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:4,padding:"8px 0",fontFamily:"inherit"}}>← {t.back}</button>
        </div>
...
function SettingsScreen({ lang, units, onSwitchLang, onSetUnits, onClearData, onBack, t, nickname, onSetNickname }) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0a0a14"}} {...swipeBack}>
      {/* safe area */}
      <div style={{height:"max(env(safe-area-inset-top,54px),54px)"}}/>
      {/* sticky back button */}
      <div style={{position:"sticky",top:0,zIndex:10,background:"#0a0a14",padding:"4px 16px 4px"}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#FF6B35",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:4,padding:"8px 0",fontFamily:"inherit"}}>← {t.back}</button>
      </div>
